...
1.) Create a calculated field that returns the maximum date (the oldest) for the selected datetime field, regardless of any filters, using the FIXED function for this purpose.
...
2.) Create a calculated field that returns the maximum day difference between the date field from the database and today's date.

Name: _Shipment Created At DateDiff INT MAX_

```python
IF INT(DATEDIFF('day', [Created At (Shipment)], TODAY())) = {FIXED : MIN([Shipment Created At DateDiff INT])} THEN
    0
ELSE
    {FIXED :MAX([Shipment Created At DateDiff INT])}
END
...
3.) Create a parameter, where for properties use the field created from step 1. and for а range of values use the field created from step 3.

<img src="https://raw.githubusercontent.com/JanCinis/jancinis.github.io/main/assets/img_post_04/Picture02.png" height="500">

4.) Create a calculated field as a Boolean type which checks that the date difference for the selected datetime field and today’s date is not higher than the number of days from the created parameter.
...
5.) Place the created field from step 4. to all worksheets where you need such a filter and choose True.

<img src="https://raw.githubusercontent.com/JanCinis/jancinis.github.io/main/assets/img_post_04/Picture03.png" height="150">

6.) Show the created parameter on the dashboard layout.
...
<img src="https://raw.githubusercontent.com/JanCinis/jancinis.github.io/main/assets/img_post_04/Picture13.png" height="450">
...
**IV. Create a calculated field to return the latest category value per consumer and date**

- What could it be useful for?

If the database has a category field that contains different values for each consumer (such as the consumer's status), which may change over time, then this calculation can be used to return the status that the consumer most recently received.
...
**V. Create a calculated field to exclude certain values from view without filtering it**
...
<img src="https://raw.githubusercontent.com/JanCinis/jancinis.github.io/main/assets/img_post_04/Picture17.png" height="300">
...
#0.0%; (#0.0" %"); #
```
<img src="https://raw.githubusercontent.com/JanCinis/jancinis.github.io/main/assets/img_post_04/Picture19.png" height="350">

As а result, all 0% are gone from the view.

&nbsp;

**VI. Hiding zero values on charts**
...
Thank you for reading and your interest! In case of feedback or questions feel free to reach out via Linkedin.
